...
//get price is done confirmed
       ...
       ///// exepted   !=
        ...
        foreach($product_expected as $profit){
            ...
            $details_expected =($profit->product->selling_price*$profit->quantity  ) +$details_expected;
        }}
        ...
        //pendingsales
        $pendingorder    =Order::where('order_status_id',  3)->pluck('id')->toarray();
        $pendingsales_order = OrderItem::wherein('order_id', $pendingorder)->get();
        $pendingsales_expected =0;
        foreach($pendingsales_order as $profit){
            if($profit->product->seller_id==$seller_id) {
            $pendingsales_expected = $profit->count('id');
        }}
       ...
       // $wallets_ex=wallet::where('seller_id',$seller_id)->where()->get();
        ...
        return view('store.wallet' ,compact('profit','balnce_expected','wallets','pinding','total','pendingsales_expected'));
    }
    public function create(Request $request){
        ...
        $this->validate($request, [ 
            'price' => 'required|numeric|max:' . $total,
            'phone' => 'required',
            'payway' => 'required',
